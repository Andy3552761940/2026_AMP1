...
# Default AMP task (kept for backward compatibility)
NUM_LABELS = 5
...
class ESMForMultiLabel(nn.Module):
    """ESM2 backbone + mean pooling + multi-label classification head.

    Notes
    - This module is used by the AMP pipeline (5 labels) but also supports
      arbitrary `num_labels` for other peptide properties (e.g., Hemolysis).
    - Forward returns a dict with keys: {loss, logits}.
    """

    def __init__(self, model_dir: str, hidden_dropout: float = 0.1, num_labels: int = NUM_LABELS):
        super().__init__()
        ...
        self.classifier = nn.Linear(hid, self.num_labels)
...
def load_model_for_inference(
    ...
    if is_adapter_checkpoint(ckpt_dir):
        ...
        model = ESMForMultiLabel(base_model_dir, num_labels=num_labels)
    ...
    if os.path.isfile(os.path.join(ckpt_dir, "config.json")):
        return ESMForMultiLabel(ckpt_dir, num_labels=num_labels)
